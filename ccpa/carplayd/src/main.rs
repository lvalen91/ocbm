//! carplayd — box-side AirPlay/CarPlay pairing daemon (armv7/musl).
//!
...
/// `btd` (`mfi_local::MFI_LOCK_PATH`) and `receiver`'s tunnel path
/// (`mfi-i2c-local::MFI_LOCK_PATH`). All four must agree on this path.
///
/// QC 2026-07-25 (HIGH): carplayd was the ONE chip user that took no lock at all. Both sequences
/// below are stateful multi-register transactions (cert = 0x30 then 0x31; sign = write 0x20/0x21,
/// trigger 0x10, poll 0x10 bit4, read 0x11/0x12), and carplayd + btd are BOTH live around
    ...
    fn create_signature(&mut self, digest: &[u8]) -> io::Result<Vec<u8>> {
        ...
        while poll_start.elapsed() < SIGN_POLL_DEADLINE {
            ...
            if self.rd(0x10, &mut st) && (st[0] & 0x10) != 0 {
                if st[0] != 0x10 {
                    eprintln!("[carplayd] mfi: sign status 0x{:02x} (expected 0x10)", st[0]);
                }
                done = true;
                break;
            }
...
impl DiskPeers {
    fn new() -> Self {
        ...
        match std::fs::read(&path) {
            Ok(bytes) => {
                ...
                eprintln!("[carplayd] peerstore: loaded {n} known device(s) from {}", path.display());
            }
            Err(_) => eprintln!("[carplayd] peerstore: none at {} (fresh)", path.display()),
        }
    ...
    fn persist(&self, map: &HashMap<Vec<u8>, [u8; 32]>) {
        ...
        if let Err(e) = write_then_rename() {
            ...
            eprintln!("[carplayd] peerstore: persist to {} failed: {e} (in-memory only)", self.path.display());
        }
    }
}
...
impl PeerSaver for DiskPeers {
    fn save_peer(&mut self, id: &[u8], ltpk: [u8; 32]) {
        ...
        let snapshot = {
            ...
            eprintln!("[carplayd] pair-setup: peer saved ({}-byte id) → persisting", id.len());
            m.clone()
        };
...
fn mfi_selftest() {
    let mut signer = LocalMfiSigner::open().unwrap_or_else(|e| {
        eprintln!("[carplayd] open /dev/i2c-1: {e}");
        std::process::exit(1);
    });
    let remote = mfi_wire::client::addr();
    match signer.device_version() {
        Some(v) => println!("[carplayd] MFi chip open (local i2c), DeviceVersion=0x{v:02x}"),
        ...
        None if remote.is_some() => {
            println!("[carplayd] MFi via remote coprocessor — DeviceVersion not available, skipped")
        }
        None => {
            eprintln!("[carplayd] MFi DeviceVersion read failed");
            std::process::exit(1);
        }
    }
    match signer.copy_certificate() {
        Ok(cert) => println!("[carplayd] copy_certificate: {} bytes  first8={}", cert.len(), hex8(&cert)),
        Err(e) => {
            eprintln!("[carplayd] copy_certificate failed: {e}");
            std::process::exit(1);
        }
    }
    match signer.create_signature(&[0x5Au8; 20]) {
        Ok(sig) => println!("[carplayd] create_signature(20B digest): {} bytes  first8={}", sig.len(), hex8(&sig)),
        Err(e) => {
            eprintln!("[carplayd] create_signature failed: {e}");
            std::process::exit(1);
        }
    }
    println!("[carplayd] LOCAL MfiSigner OK");
}
...
/// Accessory identity. The mDNS advertisement (src/discovery.rs, in this process since the
/// rx-connect merge) is built from these same values, so the iPhone associates the pairing
...
/// btd now derives a per-box identity from hardware and passes it via env; carplayd (and
/// discovery) read it here, falling back to the historical fixed values when unset (the wired path,
...
/// The Ed25519 accessory-key seed. From `CARPLAY_ED_SEED` (64 hex chars) when btd supplies
...
/// subscribed (idle) → carplayd falls back to the interim built-in default below.
...
/// points us at it. carplayd runs as root there, so reading an app-private path is fine.
///
/// Same opt-in shape as every other Pi-port override (`CARPLAY_MFI_ADDR`, `BOX_STATE_DIR`,
/// `BOX_HOSTAPD_CONF`): unset, the CCPA behaves exactly as before. Resolved once and cached —
...
/// cached: carplayd re-reads the whole config per control connection anyway, so this stays consistent
...
fn clear_levers() {
    ...
    *plock(&CONTACTS) = Default::default();
    ...
    // updateViewArea animation duration: no env form (same reasoning as the trio above — the app
    // owns it), so it clears to the constant 3000 that the box always sent before it was pushable.
    levers::set_view_area_anim_ms(levers::VIEW_AREA_ANIM_MS_DEFAULT);
...
fn load_device_config() -> (DeviceConfig, u32) {
    ...
    let dev = match std::fs::read(cfg_path) {
        Ok(bytes) if !bytes.is_empty() && bytes.len() <= MAX_CFG_BYTES => match VehicleConfig::from_yaml(&bytes) {
            Ok(vc) => {
                ...
                *plock(&CONTACTS) = Default::default();
                ...
                // updateViewArea animation duration from the top-level `view_area_anim_ms` (OUR
                // extension; absent = 3000). The lever clamps to 0..=10000 and returns what it
                // stored: out of range is LOGGED, not rejected — a cosmetic knob must never take
                // the owner's whole document off the air. The value actually sent is logged again
                // by events.rs at every switch, since iOS never acknowledges the field.
                let anim_req = vc.view_area_anim_ms();
                let anim_ms = levers::set_view_area_anim_ms(anim_req);
                if anim_ms != anim_req {
                    eprintln!(
                        "[carplayd] view_area_anim_ms={anim_req} is outside {:?} — CLAMPED to {anim_ms} ms",
                        levers::VIEW_AREA_ANIM_MS_RANGE
                    );
                }
                ...
                println!(
                    ...
                    "[carplayd] cfg: {cfg_path} ({} B) → {}×{}@{} hevc={} dpad={} viewAreas={} viewAreaAnim={}ms mainBuffered={} safe={:?}",
                    bytes.len(),
                    dev.display_width,
                    dev.display_height,
                    dev.max_fps,
            ...
            Err(e) => {
                ...
                eprintln!("[carplayd] *** PUSHED CONFIG REJECTED — {e} ***");
                eprintln!(
                    "[carplayd] *** the ENTIRE document was discarded, not just the bad key: \
        ...
        Ok(bytes) if bytes.is_empty() => {
            eprintln!(
                "[carplayd] *** PUSHED CONFIG IS EMPTY (0 bytes) — falling back to compiled \
        ...
        Ok(bytes) => {
            eprintln!(
                "[carplayd] *** PUSHED CONFIG REFUSED: {} bytes exceeds the {MAX_CFG_BYTES}-byte \
        ...
        Err(e) if e.kind() != std::io::ErrorKind::NotFound => {
            eprintln!("[carplayd] *** cannot read {cfg_path}: {e} — compiled defaults ***");
            clear_levers();
            base
        }
        ...
        Err(_) => {
            if accessory_host_attached() {
                eprintln!(
                    "[carplayd] *** NO PUSHED CONFIG at {cfg_path} but a host IS attached ({} \
    ...
    std::thread::spawn(|| {
        let listener = match TcpListener::bind("127.0.0.1:9110") {
            ...
            Err(e) => {
                ...
                // carplayd (a duplicate ARM / respawn race) whose predecessor still holds the port,
                ...
                // the stale carplayd first, so exiting IS the repair, not a crash loop.
                eprintln!("[carplayd] HID input ingest bind 127.0.0.1:9110 failed: {e}");
                eprintln!("[carplayd] FATAL: no HID input seam (duplicate carplayd?) — exiting for re-ARM");
                std::process::exit(1);
            }
        };
        eprintln!("[carplayd] HID input ingest on 127.0.0.1:9110 (task #20)");
...
/// The wired supervisor reads this to learn which bearer owns the live carplayd session, so it never
...
fn run_pairing_server() -> io::Result<()> {
    ...
    println!(
        "[carplayd] pairing server on {listen} · device 'CarLink' ({}) · setup 3939 · \
         config {} (per-connection)",
        device_id(),
        carplay_cfg_file()
    );

    // Discovery (was the separate `rx-connect` binary until 2026-09-08) starts HERE, after the bind
    // and before the accept loop, so we never advertise a receiver whose RTSP port is not yet
    // listening. It is handed the identity rather than re-deriving it: the advertised deviceid/pi
    // must equal the ones `Identity` above verifies with, and two processes reading the same env
    // vars was the only thing that ever kept them in step.
    discovery::start(device_id().to_string(), pairing_identity().to_string());
        ...
        for conn in listener.incoming() {
            ...
            generation += 1;
            {
                ...
                if let Some((_, s)) = cur.as_ref() {
                    ...
                    eprintln!(
                        "[carplayd] hijack: new control connection preempts the current session"
                    );
                }
    ...
    while let Ok((mut generation, mut stream)) = rx.recv() {
        ...
        while let Ok((newer_gen, newer)) = rx.try_recv() {
            ...
            eprintln!("[carplayd] hijack: superseded a queued control connection with a newer one");
            generation = newer_gen;
            stream = newer;
        }
        let peer = stream.peer_addr().map(|a| a.to_string()).unwrap_or_default();
        println!("[carplayd] ── control connection from {peer} ──");
        ...
        match arm_keepalive(&stream) {
            Ok(()) => eprintln!("[carplayd] control keepalive armed (3s/3s/3 → ~12s dead-link detect)"),
            Err(e) => eprintln!("[carplayd] keepalive arm failed: {e}"),
        }
        let signer = match LocalMfiSigner::open() {
            Ok(s) => s,
            Err(e) => {
                eprintln!("[carplayd] MFi open: {e}");
        ...
        let session: Box<dyn receiver::session::SessionDelegate> =
            if levers::appsetup() && receiver::relay::seam_up() {
                eprintln!(
                    "[carplayd] app-driven SETUP armed for this connection (relay seam up, {})",
        ...
        match result {
            Ok(()) => match server.session_secret() {
                ...
                Some(_secret) => println!("[carplayd] *** PAIR-VERIFY COMPLETE — session secret derived ***"),
                None => println!("[carplayd] connection closed (encrypted={})", server.is_encrypted()),
            },
            Err(e) => eprintln!("[carplayd] connection error: {e}"),
        }
    }
    Ok(())
}
...
fn main() {
    ...
    if std::env::args().nth(1).as_deref() == Some("mfi") {
        ...
        if let Err(e) = run_pairing_server() {
            eprintln!("[carplayd] pairing server failed: {e}");
            std::process::exit(1);
        }
    }
}
