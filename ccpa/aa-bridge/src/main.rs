...
fn main() {
    eprintln!("[aa-bridge {}] start (AOAP -> Android Auto USB bridge, tcp :{TCP_PORT})", ts());
    ...
    // AOAP-switch the phone and claim the owner flag BEFORE accepting, not inside the session.
    //
    // The host app no longer connects on a hand-set env var: it waits for the box's projection-mode
    // event (CT_PROJ_MODE), which ocbmd mirrors from the very flag this claims. Preparing lazily on
    // the first client therefore DEADLOCKS — the app waits for the mode, the box waits for the app.
    // Claiming here also keeps the flag's meaning honest: `wired-aa` means a LIVE accessory link
    // exists, which is exactly what makes the app's AA engine worth starting.
    //
    // The claim is why every wait below is BOUNDED by the preconditions rather than blocking: while
    // `wired-aa` is set, session_supervisor's arm()/kill_session()/escalate() all stand down, and its
    // only self-heal is `pgrep aa-bridge` — a process parked forever in a blocking accept() would
    // hold the claim, alive, and lock CarPlay out of the box until someone killed it by hand.
    if let Err(e) = listener.set_nonblocking(true) {
        eprintln!("[aa-bridge] set_nonblocking failed: {e}");
        std::process::exit(1);
    }
    ...
    loop {
        ...
        if carplay_owns() {
            eprintln!("[aa-bridge {}] CarPlay owns the box — standing down, exiting", ts());
            std::process::exit(0); // NOT clear_owner: the flag is CarPlay's, not ours to clear
        }
        if !cfg::aa_enabled() {
        ...
        if carplay_owns() {
            eprintln!("[aa-bridge {}] CarPlay armed while we prepared — standing down, exiting", ts());
        ...
        match accept_while_wanted(&listener, Some(ANNOUNCE_WINDOW), None) {
            Wait::Client(client) => {
            ...
            Wait::Idle => {
                ...
                match accept_while_wanted(&listener, Some(UNCLAIMED_RETRY), watch) {
                    Wait::Client(client) => {
                        ...
                        match prepare_accessory() {
                            Ok(acc2) if carplay_owns() => {
                                eprintln!("[aa-bridge {}] CarPlay owns the box — refusing this client", ts());
...
fn accept_while_wanted(
    ...
    loop {
        match listener.accept() {
            Ok((c, _)) => {
                c.set_nonblocking(false).ok(); // the pump is blocking I/O
        ...
        if carplay_owns() {
            eprintln!("[aa-bridge {}] CarPlay took the box while waiting — standing down", ts());
            return Wait::Gone;
        }
        ...
        if let Some(d) = deadline {
            if Instant::now() >= d {
                // One last accept before giving up: a connect that landed in the backlog during the
...
fn carplay_owns() -> bool {
    flags::owner().is_carplay()
}
...
fn phone_on_bus() -> bool {
    usb::enumerate_bus(USB_BUS)
        .iter()
        .any(|d| phone::classify_dev(d.vid, d.class) == PhoneType::Android || ACCESSORY_PIDS.contains(&d.pid))
}
...
fn serve_session(client: TcpStream, acc: Accessory) -> Result<(), String> {
    ...
    thread::spawn(move || {
        while alive_w.load(Ordering::Relaxed) {
            ...
            // that stands `arm()` down, i.e. the setting silently lied (docs/host/02_ANDROID_AUTO.md F3).
...
fn wait_for_android_phone(timeout: Duration) -> Option<usb::BusDevice> {
    ...
    loop {
        if let Some(dev) = usb::enumerate_bus(USB_BUS).into_iter().find(|d| {
            ...
            phone::classify_dev(d.vid, d.class) == PhoneType::Android && !ACCESSORY_PIDS.contains(&d.pid)
        }) {
            return Some(dev);
        }
...
fn find_accessory() -> Option<usb::BusDevice> {
    usb::enumerate_bus(USB_BUS)
        .into_iter()
        .find(|d| ACCESSORY_PIDS.contains(&d.pid))
}
