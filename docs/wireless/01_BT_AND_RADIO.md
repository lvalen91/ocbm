...
**Deliberately not re-issued:** CarPlay carries all audio — including telephony — over the AirPlay/WiFi
session, never over SCO/HFP, so nothing in the proven path depends on them. If HFP-over-BT is ever
actually exercised, restore them after the `up`.
