...
`ocbmd` and was written up in docs/host/02_ANDROID_AUTO.md and docs/host/02_ANDROID_AUTO.md, but never in the wire spec that is supposed to be
...
- The `0x19` row is now written from `crates/ocbm-proto/src/lib.rs` (`CT_PROJ_MODE`, `PM_*`) and
  docs/host/02_ANDROID_AUTO.mde, including the reserved `PM_WIRELESS_AA`.
...
**⚠️ CLOSED 2026-07-17 (docs/wireless/00_WIRELESS_CARPLAY.md), annotated 2026-08-10, paths qualified 2026-08-16. All seven items
...
`rfcomm.rs` + `sdp_server.rs` + `ssp_agent.rs`; 5 discovery -> the `rx-connect` crate (`RX_IFACE=wlan0`);
...
**3 s → 10 s** on 2026-07-25 (audit QC #428, `const HEARTBEAT_GRACE`); `STOP_GRACE = 5 s` (clean-`CT_STOP`
hold, `const STOP_GRACE`) and `REARM_HOLD = 2 s` (`const REARM_HOLD`) were added later — cite the
constant names, not line numbers, which have already drifted once (`:561/:566/:579` → `:562/:567/:580`).
