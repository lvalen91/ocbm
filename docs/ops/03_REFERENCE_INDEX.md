...
* RFCOMM is implemented **in userspace over L2CAP** there (`wireless/src/rfcomm_uspace.rs`) — the
  AAOS kernel ships without `CONFIG_BT_RFCOMM`.
* `hciconfig` does not exist on Android; bring-up is native ioctls + raw HCI (`wireless/src/hci.rs`),
...
the AA material was described only inside `docs/host/02_ANDROID_AUTO.md`, so a session that started from this document
...
AA protocol at all — `ccpa/aa-bridge` is an AOAP byte pump (`docs/host/02_ANDROID_AUTO.md`, `docs/host/02_ANDROID_AUTO.md`).
