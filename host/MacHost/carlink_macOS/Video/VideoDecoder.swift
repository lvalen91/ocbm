...
final class VideoDecoder: @unchecked Sendable {
    ...
    /// Session-end clear (2026-09-09): take the LAST PRESENTED frame off the display layer, not only
    /// the pending ones. Distinct from `flush()` above, which is the decode-side recovery: measured on
    /// this SDK, `Receiver.flush()` leaves the displayed image in place, so a session that ended with
    /// `flush()` kept its last frame on screen — a frozen CarPlay screen that looked live after an
    /// adapter restart or an NCM switch. `Receiver.flush(removingDisplayedImage:)` is the one call under
    /// the synchronizer model that removes the image, and it is `async`-only.
    ///
    /// Queue ownership (see `renderQueue`) is preserved rather than bent: the async call runs while
    /// renderQueue is parked on it, so the receiver is never touched concurrently with
    /// `enqueueImmediately` — renderQueue lends its turn, no fourth party takes one. The wait is
    /// bounded; a timeout is logged, not fatal (the layer is also hidden by the caller).
    ///
    /// Strong `self` on purpose: `endSession` drops the last reference on the line after this call,
    /// and a `[weak self]` hop would skip the clear silently — the old `mainDecoder?.flush()` there was
    /// exactly that no-op. The decoder outlives the call by the few ms the clear takes.
    func clearPresentedFrame() {
        decodeQueue.async {
            self.frameCount = 0
            self.pendingDecode.withLock { $0.removeAll() }
            self.renderQueue.async {
                self.pendingEnqueue.withLock { $0.removeAll() }
                self.frameTap?.flush()
                let done = DispatchSemaphore(value: 0)
                Task(priority: .userInteractive) {
                    await self.receiver.flush(removingDisplayedImage: true)
                    done.signal()
                }
                if done.wait(timeout: .now() + 1.0) == .timedOut {
                    Self.logger.error("[\(self.label, privacy: .public)] flush(removingDisplayedImage:) did not complete in 1 s — the last frame may still be presented")
                }
            }
        }
    }
