...
final class CarPlayView: NSView {
    ...
    /// The session is over — put the loading presentation back (2026-09-09). The ONE view-side entry
    /// point for every teardown (AppDelegate.endSession, stopAAOverOCBM): hides the video layer and
    /// shows the status overlay with a truthful line. Hiding alone is presentation-only — the layer
    /// keeps its last frame and shows it again on unhide (measured) — so callers also clear the
    /// decoder's presented frame (`VideoDecoder.clearPresentedFrame`); this method is the part that
    /// takes effect on the very next display refresh regardless of how long that takes.
    func presentSessionEnded(status: String) {
        updateStatus(status)
        setStreaming(false)
    }
