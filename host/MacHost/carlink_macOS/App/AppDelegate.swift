...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func stopAAOverOCBM(reason: String, drainTimeout: TimeInterval = 0.5) {
        ...
        // Take the AA decoder's last frame OFF the screen, not just stop feeding it (2026-09-09): a
        // plain flush leaves the presented image, and this layer is what the user is looking at.
        mainDecoder?.clearPresentedFrame()
        let view = windowController.carPlayView
        ...
        // The parked CarPlay layer comes back holding the last frame of the CarPlay session that
        // PRECEDED AA (measured: remove + re-add restores it). Clear it too, or the first unhide on
        // the next CarPlay attach flashes a screen from minutes ago before the fresh IDR lands.
        parkedCarPlayDecoder?.clearPresentedFrame()
        mainDecoder = parkedCarPlayDecoder
        parkedCarPlayDecoder = nil
        parkedCarPlayLayer = nil
        // Back to the loading presentation NOW — the CarPlay lane has no A/V at this instant, and the
        // coordinator's own setStreaming(false) never reached the view while AA held it (the guard in
        // onStreaming). The resync below re-drives the coordinator so its next progressing tick
        // re-announces streaming; when this runs from endSession the resync's hop is dropped by the
        // coordinator-identity guard and endSession's own "Waiting for adapter" wins.
        view.presentSessionEnded(status: "Waiting for phone…")
        ocbmCoordinator?.resyncAfterAAHandback()
    ...
    @MainActor private func endSession() {
        ...
        setupSession = nil
        transport = nil
        // clearPresentedFrame, not flush (2026-09-09): flush() keeps the presented image, so the last
        // CarPlay frame stayed on screen after an adapter restart / NCM switch, looking like a live
        // session. It also captured `self` weakly, so with the reference dropped on the next line it
        // usually never ran at all.
        mainDecoder?.clearPresentedFrame()
        mainDecoder = nil   // symmetric with altDecoder — don't keep a stale VideoToolbox session alive
        altDecoder?.clearPresentedFrame()
        altDecoder = nil
        ...
        // Back to the loading screen (2026-09-09). Every path that ends here — USB gone (unplug, adapter
        // restart, NCM switch), transport read-errors, Reset Session, resolution re-init — used to rely
        // on the coordinator's onStreaming(false) to hide the video layer, and none of those paths
        // produces one (the coordinator is nilled above; no phone-ABSENT arrives when the BOX goes).
        // The last frame stayed up, indistinguishable from a live session. "Waiting for adapter" is
        // the truthful state for all of them: a re-init re-enumerates, an unplug awaits the replug.
        windowController.carPlayView.presentSessionEnded(status: "Waiting for adapter...")
    }
    ...
    private func setupDevice(service: io_service_t) {
        ...
        // Every coordinator callback checks that `coord` is STILL the live coordinator (2026-09-09).
        // They fire from main-queue `hop` blocks that can land after endSession nilled this one — and
        // after setupDevice built the next session. A stale onStreaming(false) landing on the new
        // session would hide its video layer, and the new coordinator would never unhide it
        // (`if !self.streaming` is already true there): the same out-of-order family as `usbEventGen`.
        coord.onStatus = { [weak self, weak coord] s in
            guard let self, let coord, coord === self.ocbmCoordinator else { return }
        ...
        coord.onStatusDetail = { [weak self, weak coord] d in
            guard let self, let coord, coord === self.ocbmCoordinator else { return }
            if self.aaSession != nil || self.parkedCarPlayDecoder != nil { return }
            self.windowController.carPlayView.updateStatusDetail(self.activePairingCode == nil ? d : nil)
        }
        coord.onStreaming = { [weak self, weak coord] on in
            guard let self, let coord, coord === self.ocbmCoordinator else { return }
            ...
            // A/V stopped on a link that is still up — phone ABSENT, host GONE (subscription loss),
            // or the 8 s stall with the phone gone: every one is the end of THIS projection, and what
            // follows is a fresh one that starts on an IDR. setStreaming(false) only hid the layer;
            // take the frame out too, or the unhide on the next attach shows the old screen until
            // that IDR decodes (2026-09-09). Both lanes: the Nav/Alt window stays open across this.
            if !on {
                self.mainDecoder?.clearPresentedFrame()
                self.altDecoder?.clearPresentedFrame()
            }
        ...
        coord.onTransportLost = { [weak self, weak coord] reason in
            guard let self, let coord, coord === self.ocbmCoordinator else { return }
            self.handleOCBMTransportLost(reason: reason)
        }
