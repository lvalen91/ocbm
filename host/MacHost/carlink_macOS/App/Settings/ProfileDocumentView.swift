...
struct ProfileDocumentView: View {
    ...
    /// DESIGN.md's ten-word rule (owner, 2026-09-08): the file-format line used to run the extension,
    /// schema version AND the "loading replaces every value" caveat into one always-visible 21-word
    /// sentence. The extension/version stays inline (5 words); the caveat moves behind this (i).
    ...
    var body: some View {
        ...
        // The file format, once, so an owner knows what to hand-edit and what Import accepts. Split
        // per the ten-word rule: the fact (extension + schema version, 5 words) stays inline; the
        // caveat about what loading does to the form (16 words) sits behind the (i), same pattern
        // `FieldPopover` uses elsewhere in Settings for a plain-text info key.
        HStack(spacing: 4) {
            Text("JSON, .\(VehicleProfileDocument.fileExtension), schema v\(VehicleProfileDocument.currentSchemaVersion).")
                .font(.caption).foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
            Button { showFormatInfo.toggle() } label: {
                Image(systemName: "info.circle").font(.caption)
            }
            .buttonStyle(.plain)
            .accessibilityLabel("About the profile document format")
            .popover(isPresented: $showFormatInfo, arrowEdge: .trailing) {
                Text("Loading a document or a preset replaces every value in the form; press Save to push it.")
                    .font(.callout)
                    .padding(12)
                    .frame(width: 280)
            }
