...
> Phase 3".
>
> **RESOLVED 2026-09-08 — the Save decision is settled: AUTO-SAVE, no Save control (§11.9 option 3).**
> The owner had already settled the window (non-resizable, pane-sized), the pane switcher (a toolbar,
> macOS 27 tabs role) and the collapse policy (all collapsed, no persistence). Save is now settled
> too, in favour of the only fully HIG-native answer. **It does NOT ride along with Phase 3.** §11.9
> states, and this decision does not overturn, that auto-save is NOT cosmetic: this app batches edits
> and defers the push while a projection session is live, so removing the control changes WHEN config
> reaches the adapter. It gets its own design pass, its own owner sign-off, and box-side review before
> any code moves. Phase 3 ships with both Save buttons and both `⌘S` bindings untouched.
>
...
### Diagnostics — `DiagnosticsTab`, in its own file: the Box Log stream toggle + cap, and **Stream
Performance**. **AMENDED 2026-09-09:** this section read "and nothing else" until the owner moved
`StreamPerfSection` here from the Adapter tab. The move was free — `StreamPerfSection`
(`App/StreamMetricsMonitor.swift:451`) observes the `StreamMetricsMonitor.shared` singleton with its
own 1 Hz timer and has no dependency on the Adapter tab's `store`, `model`, or any adapter-connection
state, so nothing was duplicated and no busy/refresh concept was invented on Diagnostics. Note the
consequence for §11.6 risk 1: `.disabled(store.busy)` on the Adapter tab now wraps TWO trees, the
`boxState` disclosure and `boxControls`, not three. Content unchanged by Phase 3 (two controls, no density problem to solve); it does NOT acquire
...
these blocks VERBATIM from `host/MacHost/carlink_macOS/App/SettingsWindow.swift` by string
...
except `wifiAccessPoint` and `viewAreaAnimMs` (§2 stub rule).
...
| `viewAreaAnimMs` | Int | `viewAreaAnimMs` | 3000 | none (2026-09-09); emitter adds top-level `view_area_anim_ms: <n>` ONLY when n ≠ 3000; 0…10000 clamped in `clampInPlace`; carried in `CarPlayExtensions.viewAreaAnimMs`; in `exclusiveKeys(.insets, on: .carPlay)` |
...
### THE TEN-WORD RULE (owner, 2026-09-08) — binding on the ENTIRE Settings structure

**SCOPE CLARIFIED by the owner the same day: this rule applies across the whole Settings window, not
just the Vehicle tab where it was first raised.** Every file that renders any part of Settings is
bound: `VehicleTab.swift`, `AdapterTab.swift`, `DiagnosticsTab.swift`, `FeatureBadges.swift`,
`FieldInfo.swift`, `ProfileDocumentView.swift`, and `SettingsRootView`/`SettingsWindowController` in
`SettingsWindow.swift`. It also binds any string those files render from elsewhere — a
`FeatureMatrix.swift` `effect:` or `Verification.note` is unbounded IN THE POPOVER but bound the
moment a view renders it inline. New controls added later inherit the rule; it is not a one-off
cleanup.

**No explanatory text longer than TEN WORDS may render inline. Over ten words: put it behind an
(i)/badge popover, or condense it, or simplify the control until it needs less explanation.**

Issued against a screenshot of the landed Phase 3 build, where a `FeatureSupport.effect` row under
"Head unit name" rendered ~55 words inline ("Stored as the config document's name only. iOS shows
`CarLink-<suffix>` — the base is hardcoded on the box…"). The owner's words: *"That is too messy,
noisy, cluttered."*

This is STRICTER than the HIG guidance in §11.3 (Apple caps hover help at 60–75 CHARACTERS but says
nothing about inline length) and it OVERRIDES any looser reading elsewhere in this document.

What it binds, in order of how badly they violate it today:
1. **`FeatureSupport.effect` inline rows** (`FeatureExplanationRows`) — the worst offenders, 30–70
   words each, two per feature. §11.3 already routes these into `FieldPopover`; this rule makes that
   mandatory rather than a density preference. They may render inline ONLY in the `.limited` /
   `.unsupported` / `always: true` exceptions §1 names — and even then they must be condensed to ten
   words or they do not go inline at all.
2. **`Feature.summary` caption rows** — e.g. "The vehicle maker's name and logo on the projected home
   screen." is 11 words. Over. §11.3 already removes these from the resting layout.
3. **`ValueNoteLabel` rows** — fold into the popover per §11.3.
4. Any new caption, footnote or verdict string added later.

**Explicitly NOT bound by this rule** — these stay inline at whatever length they need, because
§11.4 already establishes that a warning you must hover to find is not a warning:
- the six live-computed warnings named in §11.4;
- the ⚠️ inert marker (`FieldInfo.inertMarker`);
- `PanelRule.verdict` / `viewArea2Verdict` and the preview captions;
- confirmation-dialog text, which keeps its full content.

Provenance chips (`device-proven 2026-09-04`, `unverified`) are already under ten words and are fine
as chips, but they belong in the popover beside the effect they qualify, not as standalone rows.

**AUDIT 2026-09-08 — the density claims above are CORROBORATED by the code, not merely asserted.**
Vehicle rests at ~100 form rows (16 features × heading+badge+control, plus `LiveAppearanceSection`'s
5 always-visible toggles at `VehicleTab.swift:601-616` and ~8 bottom-block rows) and exceeds 160
fully expanded. Adapter's `boxState` (`AdapterTab.swift:376-434`) alone draws ~25–30 rows with a
session up and phones paired, and the "two stacked bottom bars" are real (`AdapterTab.swift:465-488`).

**HIG TENSION, recorded knowingly (owner decision 2026-09-08).** Apple's disclosure guidance says
"Use no more than one disclosure button in a single view. Multiple disclosure buttons add complexity
and can be confusing" (`hig://general/disclosure-controls`), against §11's nine collapsible sections.
The owner chose the nine sections, on the reading that the rule addresses standalone disclosure
CONTROLS rather than `Section(isExpanded:)` inside a `Form` — which is the construct System Settings
itself uses repeatedly. The alternatives weighed and declined: merging into ~4 broader groups (would
change `Feature.Section`, a FROZEN contract file that `SettingsTests.swift` pins), and a single
"Advanced" disclosure (loses the per-topic structure entirely). Recorded so a later reader sees the
trade rather than assuming HIG endorsement that was not given.

**AMENDED 2026-09-09 — the Display section's previews are now to be REORGANISED, not just preserved.**
The rule below held through Phase 3 and is why the previews survived intact. It no longer binds the
Display section. The owner's verdict on the landed build: Display "is too clustered together and
needs better organization", and the ask is to CONSOLIDATE `SafeAreaPreview` and `ViewAreasPreview`
into ONE main box drawing the panel, the safe area and the second view area (resize area) together,
each in its own designated colour, with the driving fields moved next to the picture.
What is relaxed is the SHAPE of the drawing. What still binds, because it is correctness rather than
decoration: `ViewArea2Rule` verdict semantics (illegal ⇒ red, dashed, clipped, "not pushed" — the
YAML really does omit it); the two failure classes never presented alike (odd/out-of-panel/
non-positive kills the session, below-floor only blacks the area out); area [1] drawn full-bleed
because the emitter copies its `viewArea` into its `safeArea`; the cornerMasks ↔ safe-area coupling
and its MAIN-ONLY scope; the preview clamping nothing and re-deriving no rule; and the two claims
REFUTED on hardware (§11.10) staying out of the UI. The rest of the tree — the OEM icon thumbnail,
`PreviewCanvas`'s shared sizing convention — is still governed by the rule as written below.

**THE PREVIEW BOXES ARE A HARD CONSTRAINT (owner, 2026-09-08) — they survive Phase 3 intact.**
The display-representation boxes and their outlines stay as visual guides shown WHILE the parameters
they visualise are being edited. The complete inventory, from the same audit — there is nothing else
graphical in the Settings tree:

| Component | Site | Draws | Gated today |
|---|---|---|---|
| ~~`SafeAreaPreview`~~ / ~~`ViewAreasPreview`~~ | **MERGED, then RESTRUCTURED 2026-09-09** | superseded by `DisplayLayoutPreview` inside a PARAMETERISED `DisplayLayoutBox` (`VehicleTab.swift:368`) that BOTH displays call — Main Display and Second Display are peer sub-areas, each with its own titled box ("Main layout" / "Cluster layout"). The interim `SafeAreaField.showsPreview` / `previewTitle` parameters are GONE; the picture no longer lives inside `SafeAreaField` at all. | main box badges CarPlay only when the resize area draws; cluster box only when the second display is enabled |
| `PreviewCanvas` | `VehicleTab.swift:176-198` | shared sizing + legend helper (not a view) | n/a |
| `ViewArea2Field` | `VehicleTab.swift:278-324` | toggle + W×H@X,Y + verdict + the preview | inside the CarPlay sub-group |
| **OEM icon thumbnail** | `VehicleTab.swift:668-677` | 44×44pt decoded PNG + "W×H PNG" caption | inside the `branding` sub-group |

The last row was NOT named anywhere in this document before 2026-09-08; it is a fifth visual
affordance and is covered by the same constraint. **Why this is safe:** both previews are nested
INSIDE their control blocks — fields above, picture immediately below, same block — so re-parenting
`SafeAreaField` or `ViewArea2Field` into a `CollapsibleFeatureSection`/`GateReveal` carries the
preview with it automatically. The risk is ONLY a rewrite rather than a re-parent, in which case the
two call sites `VehicleTab.swift:382` and `VehicleTab.swift:317-321` must survive verbatim.
**SUPERSEDED 2026-09-09: those two call sites no longer exist.** The owner asked for the
consolidation, which IS the rewrite this sentence warned against, so the warning was overridden
deliberately rather than missed. What the rule was protecting — the validation semantics — was
carried across and re-verified on the running build (illegal ⇒ red, dashed, clipped, "not pushed",
with `viewArea2Verdict` rendered verbatim above the box). Colours changed from `Color.accentColor`
to system semantics, because the accent is user-chosen: a Red accent drew the safe area in the
colour reserved for illegal, and Graphite drew it in the panel's own grey.
`ViewAreasPreview` additionally renders validation state — orange/red by legality, dashed stroke when
illegal, clipped at the panel edge, and a legend swatch that switches to "Area 1 · not pushed" — and
that behaviour is precisely the "visual guide while adjusting" the constraint protects. None of it
may move into a popover.
...
| ~~`AACapability.negotiationNotes` ⚠️ rows (defect 4)~~ — **DEMOTED 2026-09-09, owner override: popover-only. The protected set is now FIVE, not six.** It renders as one inline cue ("⚠ 5 not expressed to Android Auto") with the summary and every note verbatim in the (i). Rationale: four always-visible advisory rows were noise at rest, and unlike the other five this one reports what a PROTOCOL does not express, not a config error the owner can fix. | `AANegotiationReadout` |
...
4. **No prose is rewritten** — **AMENDED 2026-09-08, owner granted the exception.** The rule stood
   as written until the HIG audit: Apple caps hover help at "a maximum of 60 to 75 characters" and
   states that "if you need a lot of text to describe a control, consider simplifying your interface
   design" (`hig://general/offering-help`), against this app's 300pt paragraph popovers. The owner
   chose to consolidate into `FieldPopover` AND shorten the prose toward that guidance, moving the
   detail into this document rather than the UI. So: relocation still may not INVENT wording, but
   CUTTING existing `FieldInfo.text` / `FeatureSupport.effect` strings toward the 60–75 character
   guidance is now in scope and pre-approved. Every cut string's full text must survive here or in
   `FeatureMatrix.swift`, never be deleted outright — the evidence notes and refuted-value records
   (§11.3) are load-bearing and are exempt from shortening. **The record of every string actually cut
   is §11.11**, not a comment in `FieldInfo.swift` — one home for it, not two that can drift.
5. **Per-pane sizing against a non-resizable window** (§1) is the hardest piece of Phase 3 and the
   ...
   **RESOLVED 2026-09-08 (SDK audit): the mechanism is `NSHostingSizingOptions`.** Set
   `hostingController.sizingOptions = [.intrinsicContentSize]` (`NSHostingController.sizingOptions`
   and `NSHostingView.sizingOptions`, both `@available(macOS 13.0, *)`, iOS/tvOS/watchOS/visionOS
   unavailable; the option set is declared at `SwiftUI.swiftinterface:32559-32572`). The hosting
   controller then reports its SwiftUI-computed fitting size to AppKit and the window follows.
   **CORRECTED 2026-09-08 (audit): the propagation claim below was wrong, and so was the premise.**
   This item originally said the window follows "via `preferredContentSize` propagation" — it does
   not; `NSWindow` does not observe `preferredContentSize` (popovers and sheets do). What actually
   happens is Auto Layout driven by the hosting view's intrinsic size. That distinction is not
   academic: **all three panes are `.formStyle(.grouped)` (`VehicleTab.swift:1403`,
   `AdapterTab.swift:330`, `DiagnosticsTab.swift:63`), and a grouped `Form` on macOS is a scrolling,
   List-backed container with NO intrinsic width and NO intrinsic height.** Measured at runtime
   2026-09-08 with the fixed frame removed, the Settings window came up **0 × 84 points — invisible**.
   The old `.frame(width: 460, height: 620)` was load-bearing on BOTH axes. Pinning width alone
   yields ~460 × 84, i.e. a window with no content. Whether per-pane height is achievable at all
   with a grouped `Form` is OPEN; if it is not, the honest outcome is a fixed height and a recorded
   regression against §1's "accommodates the size of the current pane", not a silent one.
   **SETTLED 2026-09-08 by measurement, and it IS the regression.** Verified with the app running:
   `sizingOptions = [.intrinsicContentSize]` cannot read a grouped `Form`'s ideal size at all — it
   reports near-zero regardless of content, and pinning only the width gave exactly the predicted
   460×84. `NSHostingController`'s automatic `preferredContentSize` propagation never fired in this
   configuration even with `sizingOptions` set (confirmed by direct measurement before the fix).
   **What shipped:** `SettingsRootView` applies `.frame(width: 460, height: selection.pane.restingHeight)`
   with a hardcoded per-pane height (`SettingsPane.restingHeight` — Vehicle 620, Adapter 520,
   Diagnostics 280) and `applyPaneChrome(_:)` calls `window?.setContentSize(...)` explicitly.
   Measured: Vehicle 460×672, Adapter 460×572, Diagnostics 460×332.
   **THE ACCEPTED REGRESSION:** the window no longer GROWS when a `Section(isExpanded:)` opens — the
   Form scrolls internally instead. §1's literal requirement (each pane gets its own height) is met;
   §11.6's stated acceptance test ("does the window settle to the right height when a section
   expands") is NOT, and cannot be with a grouped `Form`. Revisiting means abandoning
   `.formStyle(.grouped)` for a non-scrolling container, which is a bigger change than Phase 3 and
   was not taken. Recorded here rather than left as a silent difference from the design. This is orthogonal to
   the style mask: `styleMask` still must NOT gain `.resizable`, and content-driven auto-resize
   continues to work without it. The acceptance test stated above is unchanged and still applies.
6. **A window toolbar on an `NSHostingView`-hosted window** (§11.9) needs the toolbar to exist on the
   ...
   arrangement.
   **RESOLVED 2026-09-08 (SDK audit).** Confirmed against the macOS 27 SDK: there is NO bridge. A
   search of both `SwiftUI.swiftinterface` and `SwiftUICore.swiftinterface` for any
   `NSHostingView`/`NSHostingController` symbol referencing a toolbar returns ZERO matches;
   `NSWindow.toolbar` (`NSWindow.h:679`) is a plain settable AppKit property with no SwiftUI
   counterpart. SwiftUI's `.toolbar {}` attaches only through the `Scene`/`WindowGroup` lifecycle,
   which `SettingsWindowController` does not use. **Build the toolbar in AppKit and assign it:**
   `NSToolbarItemGroup.role = .tabs` (`NSToolbarItemGroupRoleTabs`, `NSToolbarItemGroup.h:37-41,84`,
   `API_AVAILABLE(macos(27.0))`), driving pane selection via `selectedIndex`/target-action.
   `Picker(...).pickerStyle(.tabs)` is real at macOS 27 but renders IN THE VIEW BODY, not in the
   `NSToolbar` — it is the wrong tool for a toolbar-hosted switcher. The negative half of this
   ("`.toolbar` never bridges") is strong inference from SDK absence rather than a quoted Apple
   sentence; verify empirically on the first build. The `⌘S` warning is MOOT for Phase 3 — auto-save
   is deferred to its own pass (§11.9) and both Save buttons stay `Form` descendants; note when that
   pass happens that `⌘S` is bound TWICE, `VehicleTab.swift:1391` AND `AdapterTab.swift:475`, not
   once as this item implied.
...
**CORRECTED 2026-09-08 — `.presentationCompactAdaptation` IS pinnable and is pinned:**
`@available(iOS 16.4, macOS 13.3, tvOS 16.4, watchOS 9.4, *)` (`SwiftUI.swiftinterface:28583-28590`).
It was listed below as unverifiable; it is not. Largely moot on macOS (no compact-width adaptation
path) but available and harmless. Three further §11.7 corrections from the same audit:
`Section(isExpanded:content:header:)`'s PUBLIC init is `@available(iOS 17.0, macOS 14.0, …)`
(`SwiftUI.swiftinterface:14568-14571`) — the table's "first-class rendering path only from macOS 26"
framing describes an internal `@usableFromInline` macOS-27 overload, not the API in use; the public
one has been available since macOS 14 and `Section.collapsible(_:)` is soft-deprecated TO it, which
confirms the choice. The `concentricCornerRadii` / `ConcentricRectangle` row is **optional polish,
not a migration**: `RoundedRectangle(cornerRadius:)` is NOT deprecated — only the `.cornerRadius(_:)`
MODIFIER is (`SwiftUICore.swiftinterface:22388-22393`, "Use `clipShape` or `fill` instead"), and
`ConcentricRectangle` is for shapes nesting inside a container's corners, which is not what the
preview boxes draw. And a trap this section missed entirely: **`@State` became a MACRO in SDK 27**
(`swiftui-whats-new-27/references/state-macro.md`). Adding `@State private var expanded = false` to
a pane view that ALSO takes init parameters — exactly the shape nine collapsible sections create —
fails with "variable used before being initialized", or silently ignores an init-time assignment in
favour of the inline default. Composing another wrapper on `@State` gives "invalid redeclaration of
synthesized property". A view whose stored properties are all private loses its synthesised
memberwise init. Reordering init assignments is NOT the fix; read that reference before writing the
collapsible views.
...
**SEMANTICS SPECIFIED 2026-09-08 (owner) — this is what auto-save MEANS here, and it closes the
"needs its own design pass" caveat below.** Three rules:

1. **Saving is automatic and unconditional.** Every edit persists immediately. No Save gate.
2. **Applying is deferred to session initialisation.** The saved config is what gets pushed to the
   box when a session starts. An edit made while a session is live does NOT disturb that session;
   it takes effect on the next one.
3. **A button forces the issue.** Where Save sits today, a control can force a new session — or
   restart the active one — so a just-made edit applies now instead of next time.

**The runtime already behaves this way; only the UI framing is wrong.** `OCBMClient.repushConfig`
(`OCBM/OCBMClient.swift:317-331`) stages `sessionConfig` unconditionally, then returns early with
"applies at the next connect" when `!subscribed`, and holds back with "config change held back —
… owns the box; applies on the next connection" when `projMode` is `pmWiredCp`/`pmWirelessCp`.
`subscribe()` (`:333-341`) sends `ctSubscribe` + `sessionConfig`, i.e. config reaches the box at
session init. `VehicleConfigModel.save()` (`App/SettingsWindow.swift:614-727`) already clamps,
persists, snapshots `committedYAML`/`committedConfig`, clears `dirty` and calls `repushConfig`.
**So auto-save is, mechanically, "call `save()` on change (debounced)" — not new plumbing.**

**What `dirty` becomes.** It does not disappear; its MEANING changes. Today `dirty`
(`SettingsWindow.swift:417`) means "form differs from UserDefaults", which under auto-save is
always false. The state that matters becomes **"staged config differs from what the live session
was started with"** — computable as `committedConfig` vs `ConfigIntegrity.shared`'s `pushed`
(`OCBM/ConfigIntegrity.swift:66-76`, the structured twin of the YAML a LANDED SUBSCRIBE carried).
That is the indicator to render, and its honest label is *"applies on the next session"*, not
*"unsaved"*.

**What the button is.** NOT a Save — it saves nothing. It is a **force re-SUBSCRIBE**, and it must
knowingly override the guard `repushConfig` applies today, because that guard exists for a real
reason: the re-SUBSCRIBE dips box-side presence, which restarts a live CarPlay session. Therefore:
- Needs a NEW entry point (e.g. `repushConfig(force:)` or `forceResubscribe()`) on `OCBMClient`.
  **`OCBMClient.swift` is outside every Phase 3 worker's scope**, which is why this stays a separate
  pass rather than riding along.
- Available only when staged ≠ pushed AND a session is live. With no session there is nothing to
  force — the config already applies at the next connect — so the control states that rather than
  presenting a dead button.
- Carries a `.confirmationDialog`, matching the existing reboot / Forget-all / Enter-NCM pattern,
  because it drops the projection. Wording stays protocol-neutral per defect 7 — name the effect
  ("the current session restarts"), not the protocol.
- Is NOT labelled "Save". Something in the shape of "Apply now — restarts the current session".
- **Lives on the ADAPTER tab, in `boxControls` beside the other session controls** (owner,
  2026-09-08) — next to Reboot / Enter NCM / Forget all (`AdapterTab.swift:441-449`), NOT on the
  Vehicle tab beside the settings that made it necessary. Rationale: every other session-disrupting
  action already lives there, each paired with the `.confirmationDialog` pattern this one needs, and
  §1 already fixes that live box state and its destructive actions stay on Adapter. The cost,
  accepted knowingly: the control is furthest from the Vehicle-tab fields that made it necessary, so
  the "applies on the next session" indicator MUST also appear on the Vehicle tab where the edit
  happens — otherwise an owner who never opens Adapter never learns their change is pending.
  The indicator is a read-only state on both tabs; only the ACTION is Adapter-only.

**Still deferred to its own pass**, and Phase 3 ships unchanged in this respect: both Save buttons
and both `⌘S` bindings stay exactly as they are until that pass lands.

---

**RESOLVED 2026-09-08 — the owner chose option 3, AUTO-SAVE, with the scoping caveat this section
already stated.** Auto-save is the only fully HIG-native answer and the only one that makes the
question disappear: the HIG audit found the word "Save" nowhere in the Settings guidance at all
(`hig://general/settings` describes only panes, a pane-switching toolbar, dynamic titles and pane
restoration), and Save/Cancel is documented ONLY for sheets (`hig://general/sheets`) — a modal,
scoped-task pattern this window is not. Option 2 (bottom bar) is the one placement Apple names as
wrong for a critical action; option 1 (toolbar) is undercut both by the HIG's statement that a
Settings toolbar "identifies the areas people can customize and helps people navigate among those
areas" — a navigation surface, not an action bar — and by the SDK finding that macOS has neither
`.topBarPinnedTrailing` nor `ToolbarOverflowMenu` (both `@available(macOS, unavailable)`).
**It does NOT ship with Phase 3.** As stated below, auto-save is NOT cosmetic — this app batches
edits and defers the push while a projection session is live, so it must first define what happens
to an edit made mid-session, or config reaches the box at a moment the current design deliberately
avoids. That needs its own design pass, owner sign-off, and box-side review. Until it lands, Phase 3
leaves both Save buttons, both bottom bars and both `⌘S` bindings exactly as they are.

**The original OPEN text is kept below as the record of the three options weighed.** With the toolbar
...
case (.insets, .carPlay): return ["enablesViewAreas", "mainDrawOutsideSafe", "enablesCornerMasks",
                                  ...
                                  "viewAreaAnimMs"]   // + resize animation ms, 2026-09-09 (§5)
```
...
### 11.11 FieldInfo shortening ledger (DESIGN.md §11.6 item 4 amendment, 2026-09-08)


The single source of truth for the §11.6 item 4 shortening pass over `FieldInfo.descriptions`
(`FieldInfo.swift`). Of the 59 entries, **55 were shortened toward Apple's 60–75 character hover-help
guidance** and 4 were already short enough to leave untouched. Every full ORIGINAL string is recorded
here, verbatim, so none was deleted outright — this is the one place that record lives; `FieldInfo.swift`
carries only the current (shortened) string and a pointer to this section, per the project's single-
source-of-truth documentation rule (root `CLAUDE.md`).

**Fact-preservation correction, 2026-09-08.** A first pass of this shortening (same day) dropped a
small number of facts that existed ONLY in the cut prose — a wire-key numeric identifier, a device-
verification claim, an operational "when this takes effect" instruction, and behavioural side effects
of a few input/audio controls not stated anywhere else in the codebase. Per the brief ("shortening
prose is authorised; deleting a fact that only existed in that prose is not"), those 9 entries were
revised a second time to keep the fact while remaining materially shorter than the original — they are
marked below. Every other shortened entry was checked against `FeatureMatrix.swift` (`FeatureSupport.effect`,
`ValueNote`) and the live in-UI verdict strings named in §11.4/§11.10; where a cut clause is a live wire
fact, it is restated there and is not lost by being shortened here.

**Second fact-preservation correction, 2026-09-08.** The shortening pass above was authorised on the
premise that the cut detail would move into a consolidated `FieldPopover` (§11.2/§11.3), so a user
would still reach it one click away. `FieldPopover` was not wired up at the time this shortening
landed — a concurrent agent is doing that now — so for the intervening period the shortened string
was the WHOLE of what a user saw, not an above-the-fold summary of something recoverable below. An
audit of the 46 non-restored entries against `FeatureMatrix.swift`, `VehicleTab.swift`,
`AdapterTab.swift` and `AA/*.swift` found 13 more facts that exist NOWHERE ELSE in the codebase: 6
dropped wire keys (`allowVideoPlayback`, `vehicleStateProtocol`, a second key on `enablesMapAppearance`,
`mainBufferedInfo`, `enhancedSiriInfo`, `logTransfer`), 2 evidence/verification claims
(`restrictionsDeclared`'s "(the proven posture)", `enablesDCX`'s simulator-only scoping), 2 sentinel/
mechanism facts (`chargingConnectorsNeutral`'s "0 = unstated", `enablesCornerMasks`'s per-corner-bitmap
mechanism), and 3 operational/behavioural facts (`panelResolution`'s "needs a fresh session",
`viewArea2Enabled`'s "both are pushed; iOS picks which is live", `metadataSkip`'s unrecognized-name
handling). Those 13 entries were revised a second time, on the same rule as the first correction: keep
the fact, stay materially shorter than the original. They are marked "(2nd pass)" below, distinct from
the 9 entries the first correction already fixed (marked "(1st pass)" where a key needed both).

| Key | Original | Shortened | Note |
|---|---|---|---|
| `name` | The name the phone knows this head unit by. Each projection renders it its own way — see the badges beside the field. Kept short: it rides every pushed configuration. | What the phone calls this head unit; each projection shows it its own way. |  |
| `driverPosition` | Which side of the car the driver sits on. Decides which edge the projected app rail and driver-focused layout favour. Android Auto takes all three values; CarPlay's key is a right-hand-drive boolean, so Centre reaches it as left. Centre has not been tried on a phone on either projection. | Which side the driver sits on; sets which edge the app rail favors. CarPlay has no Centre. |  |
| `theme` | The profile's standing choice of light or dark projected UI, or Auto to follow this Mac's own appearance. The badges say how each projection receives it. The live-session switches further down act on the connected phone now, independently of this value. | Light, dark, or Auto (follow this Mac). Live-session switches below act independently. |  |
| `statusBar` | Status-bar elements the car draws itself, so the phone should not: clock, signal strength, battery. Recorded in the profile; a projection sends it only when its wire field for it is confirmed — the badges say which. | Status-bar elements the car draws itself, so the phone should hide them. |  |
| `dpi` | Dots per inch of the panel, used by projections that scale their UI from a density rather than from the pixel size. 160 is the reference (mdpi) density; a higher value draws larger UI on the same pixel grid. | Panel density in dots per inch; 160 is the reference (mdpi) scale. |  |
| `diagonalInches` | Physical diagonal of the panel, for the record and for the implied-density check beside it. 0 = unknown. Not sent to either phone. | Physical panel diagonal, for the density check. 0 = unknown. Not sent to either phone. | Restored 2026-09-08: the first shortening pass dropped "Not sent to either phone" — a scoping fact that exists nowhere else (`diagonalInches` has no `Feature`/`FeatureMatrix` entry). |
| `restrictions` | What the phone withholds while the car is moving. Declared here; switched on and off at runtime in Window ▸ Controls. Each row says which projection can express it — a projection with no equivalent ignores that row. | What the phone withholds while driving; switched live in Window ▸ Controls. |  |
| `restrictionsDeclared` | Off = declare nothing and let each phone apply its own default restriction set (the proven posture). On = declare exactly the rows ticked below. | Off = each phone's own default restrictions (the proven posture). On = exactly what's ticked below. | Restored 2026-09-08 (2nd pass): dropped the device-verification claim "(the proven posture)". |
| `voiceRateHz` | Sample rate of the voice / assistant audio path. 48 kHz gives noticeably clearer prompts than the 16 kHz floor; the badges say which projection negotiates it directly and which reaches it through its own audio table. | Voice/assistant audio sample rate; 48 kHz is clearer than the 16 kHz floor. |  |
| `telephonyOverProjection` | Route phone-call audio over the projection link instead of the car's own Bluetooth hands-free profile. An experiment: leave off unless you are testing it. | Route call audio over the projection link instead of car Bluetooth. Experimental. |  |
| `metadataFeeds` | Which live feeds the head unit consumes from the phone: now-playing, turn-by-turn navigation, call state. Each is one service declaration on projections that declare them individually; the CarPlay tier picker below governs CarPlay's declaration as a whole. | Live feeds the head unit consumes: now-playing, navigation, call state. |  |
| `aaFitPanelWithMargins` | When the panel is not one of Android Auto's fixed video sizes, declare the enclosing size and tell the phone to keep its UI inside a panel-shaped sub-rectangle (margins), which this app crops away and scales to the panel. Off = snap to the nearest smaller size instead and letterbox. | Fit a non-tier panel with cropped margins; off snaps smaller and letterboxes. | Restored 2026-09-08 (3rd pass): the shortened text dropped "and letterbox" — the visual consequence (black bars) of the off state. Grepped `letterbox`/`pillarbox`/`black bars` across `FeatureMatrix.swift`, both tabs and `AA/*.swift`: zero hits, so the fact existed nowhere else. Found by the 2nd-pass re-audit, which the 1st audit had missed. |
| `aaPreferHEVC` | Ask the phone for H.265 even at sizes where H.264 would do. Sizes above 1080p are H.265-only regardless; this only changes the codec at and below 1080p. | Ask for H.265 at 1080p and below too (above is H.265-only regardless). |  |
| `brandingLabel` | The vehicle maker's name shown on the projected home screen. A projection with a custom-icon slot shows it beside the icon; one without shows the name alone. | The vehicle maker's name shown on the projected home screen. |  |
| `panelResolution` | The exact pixel grid of the panel the projection fills; also the space touch coordinates are reported in. A change needs a fresh session. Each side 480–3840 px, and at least 800×480 for a landscape panel or 480×800 for a portrait one (orientation from the panel's own aspect) — an out-of-range value shows in red with what Save will store, and a note under the field records any correction. | The panel's exact pixel grid; also where touch coordinates are reported. A change needs a fresh session. | Restored 2026-09-08 (2nd pass): dropped the operational fact "A change needs a fresh session". |
| `panelFrameRate` | Highest frame rate the panel is driven at. Both projections offer exactly 30 and 60; a higher rate is smoother and costs more decode and link bandwidth. | Highest frame rate the panel is driven at: 30 or 60. |  |
| `panelInsets` | Pixels from each edge that the projected UI keeps clear, for curved or occluded panel edges. The video still fills the whole panel; only interactive UI is held inside the box. 0 = flush. The projections that honour it are shown by the badges. | Edges the projected UI keeps clear, for curved or occluded corners. |  |
| `hevcAllowed` | Whether the phone may encode the projection as H.265 (HEVC) rather than H.264. Needs the decoder on this Mac, which is present; the badges say how each projection uses the permission. | Whether the phone may encode as H.265 (HEVC) instead of H.264. |  |
| `altDisplay` | A second projected surface such as an instrument cluster or a navigation strip, with its own size and rate. Leave off for a single-screen head unit. | A second projected surface, like an instrument cluster, with its own size/rate. |  |
| `primaryInputDevice` | The control surface the driver mainly uses. Choosing Touchscreen turns the touchscreen on (a touchpad may stay present beside it). Choosing Touchpad turns the touchpad on AND the touchscreen OFF — both projections then see a controller-only car; there is no way to declare a touchpad as primary next to a touchscreen. Choosing Rotary knob turns the knob on and leaves the other surfaces as they are. | Main input surface. Choosing Touchpad also turns the touchscreen off entirely. | Restored 2026-09-08: dropped the fact that choosing Touchpad turns the touchscreen OFF entirely (both projections then see a controller-only car). Not stated anywhere else. |
| `inputTouchscreen` | The car has a touchscreen. Off = no touchscreen is declared to either projection, and the phone drives its UI from the other surfaces only (focus / keycodes). Turning it on makes it the primary input unless the rotary knob is; turning it off with Touchpad present makes the touchpad primary. | Off = no touchscreen declared to either projection. On = primary input unless a knob is present. | Restored 2026-09-08: dropped the primary-input derivation (touchscreen becomes primary unless the rotary knob is). Not stated anywhere else. |
| `inputTouchMultiTouch` | The touchscreen reports two fingers at once. Default off — multi-finger gestures are untested on this project; enable deliberately and verify on hardware before shipping it. | The touchscreen reports two fingers at once. Untested — verify on hardware. |  |
| `inputTouchCancel` | The touchscreen can cancel a touch in progress (palm rejection, interrupted gestures), so an aborted touch is not taken as a tap. | The touchscreen can cancel an in-progress touch (palm rejection). |  |
| `inputTouchpad` | The car has a remote touchpad whose finger position drives a focus cursor. Usually the primary input on cars that have one. | The car has a remote touchpad driving a focus cursor. |  |
| `inputTouchpadButtons` | The touchpad also reports press/click buttons, not just position. | The touchpad also reports press/click buttons. |  |
| `inputKnob` | The car has a rotary controller: turn = scroll or rotate, press = select. Prerequisite for the two knob options. | A rotary controller: turn to scroll, press to select. |  |
| `inputKnobNudge` | The knob tilts in four directions as well as rotating, for grid navigation. | The knob tilts four ways as well as rotating, for grid navigation. |  |
| `inputDPad` | Four- or eight-way directional buttons with a select, for list navigation. | Four- or eight-way directional buttons with a select. |  |
| `engineTypesNeutral` | What powers this vehicle. A hybrid is genuinely two selections (for example gasoline AND electric). Selecting nothing leaves each projection's own default. The badges say which projection currently receives it and when it takes effect. | What powers this vehicle; a hybrid picks two (e.g. gasoline + electric). |  |
| `chargingConnectorsNeutral` | Which charging connectors this vehicle physically has, and optionally how fast each can charge (0 = unstated). Only meaningful for electric or plug-in hybrid vehicles; one row per connector type. | Which charging connectors this vehicle has, and optionally their power (0 = unstated). | Restored 2026-09-08 (2nd pass): dropped the sentinel "0 = unstated". |
| `safeArea` | Insets (px from each edge) of the box CarPlay keeps its UI inside. The video still fills the whole resolution; only interactive UI is held within the safe box — for curved/irregular panels where the corners/edges are occluded. 0 = flush (no inset). iOS honors this whenever View areas is negotiated — a real (non-zero, non-full-panel) inset here arms it automatically, even with View areas off, matching the adapter's own local behavior. | Insets CarPlay keeps interactive UI inside; 0 = flush with the panel. |  |
| `drawUIOutsideSafeArea` | Let CarPlay draw non-interactive UI in the gap between the full frame and the safe box. Off (default) = keep all UI strictly inside the safe area. | When enabled, wallpaper is displayed outside the safe area, replacing the normal black background. | Rewritten 2026-09-09 after the owner confirmed the behaviour on hardware. Both the original and the first shortening said "non-interactive UI", which is true but tells the reader nothing about what they would SEE. The flag renders the wallpaper into the inset band; off leaves it black. `docs/carplay/06_AV_PIPELINE.md` corrected in the same pass. The CONTROL was also renamed "Allow UI outside safe area" -> **"Extend wallpaper"**: the old label named the wire key (`drawUIOutsideSafeArea`) rather than the effect, and implied tappable UI moves outside the safe rectangle, which it does not. |
| `viewArea2Enabled` | Declare a SECOND main view area so the CarPlay Dock shows the resize button and the user can switch between the full panel (area 0) and this smaller rect (area 1). Both are pushed in the config; iOS picks which is live. Off = one layout, no resize button. | A second main view area; both are pushed and iOS picks which is live. Dock shows a resize button. | Restored 2026-09-08 (2nd pass): dropped "both are pushed; iOS picks which is live". |
| `viewArea2Rect` | Area 1 in panel pixels: width, height and the top-left origin. The origin is free (a corner, an edge, or floating). All four values must be EVEN and the rect must fit inside the panel — an odd value or an overflow tears the session down on hardware, so such a rect is not pushed at all. Below the product floor (800×480 landscape / 480×800 portrait, by the rect’s own aspect) CarPlay blacks the area out instead. Area 1’s safe area is the whole rect; there are no separate insets for it. | Area 1's size and origin, in even pixels, fully inside the panel. |  |
| `altResolution` | Pixel size of the secondary (instrument-cluster / navigation) video stream — typically a smaller cluster screen. Omit for single-screen units. | Pixel size of the secondary (instrument-cluster/nav) video stream. |  |
| `enablesVideoPlayback` | Advertises allowVideoPlayback so iOS can stream arbitrary fullscreen video apps (media/streaming), not just the CarPlay UI. Off = UI/nav only. | Advertises allowVideoPlayback so iOS can stream arbitrary fullscreen video apps, not just CarPlay UI. | Restored 2026-09-08 (2nd pass): dropped the wire key `allowVideoPlayback`. |
| `enablesMainBufferedAudio` | Advertises mainBufferedInfo (~2-min media buffer, streamed from iPhone faster than real time) for playback resilience on an UNINTENTIONAL drop — media keeps playing while the link recovers. Does NOT improve audio quality. Advertise/negotiate is device-tested (wired: iOS negotiates it DISABLED over USB — it is a wireless-drop remedy). The box does not serve a buffered stream yet (the ~2-min buffer will live in THIS app — docs/carplay/04_CAPABILITIES_AND_CONFIG.md's owner-corrected architectural model, 2026-08-07), so on WIRELESS this is a deliberate per-session experiment: if iOS moves media to the buffered stream, media goes silent until you turn this off and reconnect. Default off; applies at the next connection. | Advertises mainBufferedInfo: a ~2-min resilience buffer for drops. iOS disables it over wired USB; wireless can go silent — experimental. | Restored 2026-09-08 (1st pass): dropped the device-tested fact that iOS negotiates this DISABLED over wired USB, and the wireless failure mode. Restored 2026-09-08 (2nd pass): the wire key `mainBufferedInfo` was still missing. |
| `audioFormats` | The exact set of audio capabilities the box advertises to iOS (the /info audioFormats). iOS negotiates one entry per audioType from this set. Auto = match the transport (PCM over USB, the AAC set over wireless). Presets are ready-made sets; Custom lets you author any codec/rate/stream-type combination to test a specific head-unit audio config. | The exact audio capabilities the box advertises to iOS. |  |
| `metadataTier` | Which metadata feeds the accessory DECLARES to iOS in its iAP2 identification (and then subscribes to): now playing, call state, route guidance, and so on. Proven is the declaration a real iPhone accepted on 2026-07-25 and is what the adapter uses when nothing is pushed — leave it here unless you are deliberately testing a wider set. Extended adds the full paired Start/Stop set — accepted once on a wireless session's tunnel Identify (2026-07-25), but NOT proven on the wired Identify, where an earlier extended form was rejected. All declares everything in the capability table. ⚠️ iOS validates this: a declaration it rejects kills the whole identification for that connection and cannot be retried within it, so raise the tier one step at a time and watch the phone's own log (idevicesyslog -p accessoryd). Applies at the next iAP2 link — unplug/replug the phone, not just a reconnect of this app. | What the accessory declares to iOS at identification. Applies at next iAP2 link — unplug/replug, not just reconnect. | Restored 2026-09-08: dropped "applies at the next iAP2 link — unplug/replug the phone, not just a reconnect of this app", an operational fact not stated elsewhere (the per-value evidence itself IS preserved, in `FeatureMatrix.valueNotes`). |
| `accessoryName` | The name this adapter shows as on the iPhone. Leave empty to keep the adapter's built-in per-box name (CarLink plus the last four characters of its Wi-Fi address), which is what ships today and is what keeps two adapters distinguishable. A name you set is used verbatim, so make it distinct yourself. ⚠️ Not yet applied by the adapter — it is stored and pushed, but changing the advertised name touches the AirPlay /info name, the Bonjour service name and the iAP2 identification together, so it is enabled in a later step with the phone's own log being watched. | The name shown on the iPhone; default is CarLink + last 4 of the Wi-Fi address. Not yet applied by the adapter. | Restored 2026-09-08: dropped the default-name mechanism (CarLink + last 4 chars of the Wi-Fi MAC) and which three wire surfaces a set name touches. Not stated elsewhere. |
| `vehicleStatusEnabled` | Declares that this vehicle can report live status to the phone — range, temperatures, charge state and so on. ⚠️ DISABLED until the adapter can service it. The adapter does not yet declare the messages that carry this data, so announcing the capability without them is exactly the kind of inconsistency iOS rejects, and a rejected identification kills CarPlay for that connection and cannot be retried until the phone is replugged. The control is shown greyed rather than hidden so you can see the capability is planned; it will unlock when the adapter declares the messages that carry this data. | Declares live vehicle status support. Greyed — the adapter can't serve it yet. |  |
| `metadataSkip` | Comma-separated feature names to DROP from the declaration above (e.g. call_history). Use this to narrow a tier that iOS rejected, rather than dropping back a whole level. Names are the adapter's feature-table names; anything unrecognized is ignored. Leave empty for the full tier. | Feature names to drop from the declaration above (adapter's feature-table names); unrecognized ones are ignored. | Restored 2026-09-08 (2nd pass): dropped "names are the adapter's feature-table names; anything unrecognized is ignored". |
| `audioFormatRow` | One advertised capability: a stream type (100 MainAudio carries the mic; 102 MainHighAudio is high-latency media (AAC-LC)), an audioType iOS routes against (e.g. media, speechRecognition), an input codec (mic capture; None = playback-only) and an output codec (playback to the box). | One advertised capability: stream type (100 mic, 102 media), audioType, input/output codec. | Restored 2026-09-08: dropped the actual stream-type numeric identifiers (100 = MainAudio/mic, 102 = MainHighAudio/media) — a wire-key-class fact, not duplicated elsewhere. |
| `enablesEnhancedSiri` | Publishes enhancedSiriInfo so the vehicle's own hardware Siri button can invoke Siri (siriAction prewarm / button-down / button-up), with supported-language hints and mixable Siri audio. Off = basic Siri via the CarPlay Siri button. Either way Siri listens on the car's microphone (the speechRecognition audio path) — enhanced just adds the hardware-button trigger, language hints, and audio mixing. Not pursued on this project: it needs an independent hot-word / voice-analysis stack. | Publishes enhancedSiriInfo: hardware Siri-button support (basic Siri already listens via mic regardless). Not pursued. | Restored 2026-09-08 (1st pass): dropped the scoping fact that basic Siri already listens on the mic regardless. Restored 2026-09-08 (2nd pass): the wire key `enhancedSiriInfo` was still missing. |
| `enablesUIAppearance` | Advertises UI-appearance control so the head unit can drive CarPlay's look (uiAppearanceUpdate). Enabled in every Apple template. | Lets the head unit drive CarPlay's look. Enabled in every Apple template. |  |
| `enablesMapAppearance` | Advertises map-appearance control (mapAppearanceUpdate, changeMapZoomLevel); also required for the alt/cluster map stream. | Lets the head unit drive map appearance (mapAppearanceUpdate, changeMapZoomLevel); needed for the alt/cluster map stream. | Restored 2026-09-08 (2nd pass): dropped the second wire key `changeMapZoomLevel` (`mapAppearanceUpdate` survives in FeatureMatrix). |
| `enablesCornerMasks` | Declares the display can be masked at the corners (rounded/cut edges); the car streams per-corner opaque bitmaps at runtime. CarPlay's "cutout" mechanism. | Declares the display can be masked at the corners; the car streams per-corner opaque bitmaps at runtime (CarPlay's cutout). | Restored 2026-09-08 (2nd pass): dropped the mechanism "the car streams per-corner opaque bitmaps at runtime". |
| `enablesViewAreas` | Support declaring view areas / safe areas within the display (a usable sub-rectangle inside a wider panel, split layouts). | Support for view areas / safe areas within the display. |  |
| `enablesFocusTransfer` | Focus can move between CarPlay and the head unit's own UI (split screens / multi-display). Off = CarPlay keeps focus. | Focus can move between CarPlay and the head unit's own UI. |  |
| `enablesUIContext` | Publishes/updates UI context (which app/screen is showing) so the car can react. Off = no context reporting. | Publishes which app/screen is showing so the car can react. |  |
| `enablesUISync` | CarPlay and the head unit keep certain UI state in step. Off = no sync. [inferred] | Keeps certain UI state in step between CarPlay and the head unit. [inferred] |  |
| `enablesFileTransfer` | Advertises the file/asset transfer capability so iOS can push assets to the accessory. Exact wire use is not evidenced in Apple's sources. [inferred] | Lets iOS push asset files to the accessory. [inferred] |  |
| `enablesLogTransfer` | Advertises logTransfer — the accessory tells iOS it can supply a diagnostic log archive. The advertise/negotiate handshake is device-proven on the wire (wired). The box deliberately does NOT serve the archive and won't: not a privacy limit — Apple won't troubleshoot a non-conventional CarPlay implementation, so it would be wasted effort. Advertise/negotiate only; default off. | Advertises logTransfer, a diagnostic log archive (advertise/negotiate is device-proven wired). Box never serves one. | Restored 2026-09-08 (1st pass): dropped the device-verification claim. Restored 2026-09-08 (2nd pass): the wire key `logTransfer` was still missing. |
| `enablesVehicleDataProtocol` | Advertises vehicleStateProtocol, opening the two-channel Vehicle Data Protocol for route status + vehicle state (VDC). Needed for turn-by-turn cluster / nav telemetry. | Advertises vehicleStateProtocol, opening the two-channel Vehicle Data Protocol (VDC) for route status and vehicle state. | Restored 2026-09-08 (2nd pass): dropped the wire key `vehicleStateProtocol`, "two-channel", and "(VDC)". |
| `enablesDCX` | "DCX" — purpose NOT evidenced. Only the property name exists in the simulator; no CarPlaySDK string or wire mapping was found. A "dynamic content" meaning is unverified. Leave default. | "DCX" — only the property name exists in the simulator; no CarPlaySDK string or wire mapping. Leave default. | Restored 2026-09-08 (2nd pass): dropped the evidence note "only the property name exists in the simulator; no CarPlaySDK string or wire mapping". |

**Left unchanged (already ≤ ~75 characters):** `inputKnobHomeBack`, `inputMediaButtons`, `inputTelephonyButtons`, `inputSteeringWheel`.
