...
struct CarPlayExtensions: Codable, Sendable, Equatable {
    ...
    /// Dock-resize animation between view areas, ms (the box's `animationDurationMillis`; our
    /// `view_area_anim_ms:` YAML extension, emitted only when != 3000). CarPlay-only: Android Auto
    /// has no second view area. 0…10000, clamped on import by `clampInPlace`.
    var viewAreaAnimMs: Int = 3000
...
struct AdapterSettings: Codable, Sendable, Equatable {
    ...
    /// (`btd`) advertises BOTH the wireless-CarPlay and the wireless-Android-Auto SDP
