...
final class OCBMSessionCoordinator: OCBMClientDelegate, USBTransportDelegate, @unchecked Sendable {
    ...
    /// Android Auto handed the window back to CarPlay (AppDelegate.stopAAOverOCBM, 2026-09-09). While AA
    /// owned the view every onStreaming/onStatus from here was suppressed at the AppDelegate guard, so the
    /// view and this watchdog have diverged: the view may still be in "streaming" from the CarPlay
    /// session that preceded AA, and `streaming` here may be stale-true if no phone-ABSENT ever arrived
    /// (wireless). Re-drive both from the truth of this instant — the CarPlay lane has no A/V — so the
    /// next progressing stats tick re-announces streaming instead of being swallowed by
    /// `if !self.streaming`, which would leave the video layer hidden for good.
    func resyncAfterAAHandback() {
        hop {
            self.announcedWaiting = false; self.announcedIdle = false
            self.setStreaming(false)
            self.status(self.phonePresent ? "iPhone connected — starting CarPlay…" : "Waiting for phone…")
        }
    }
