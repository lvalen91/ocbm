# MacHost — the macOS host app (A/V verification milestone)

> **RENAMED 2026-09-08: `CarPlayHost/` → `MacHost/`.** Two things were wrong with the old name.
>
> 1. **It named one of two transports.** This is a **phone-projection host, not a CarPlay host**:
>    alongside the OCBM CarPlay path this doc describes, `carlink_macOS/AA/` carries a full **Android
>    Auto** client (~3.4 kLOC — `AASession`, `AATLS`, `AAWire`, `AACapability`, `AAMetadata`),
>    instantiated in `AppDelegate.setupDevice()` and switched at runtime
>    (`ControlsBridge.shared.isAndroidAuto`, `CarPlayView.isAndroidAuto`). The AA path is documented in
>    `docs/androidauto/`; everything below is the CarPlay A/V milestone record and is unaffected.
> 2. **"Carlink macOS" was not available as the replacement.** That name belongs to the *predecessor* —
>    the app built against the stock **Carlinkit riddlebox firmware**. This tree is the
>    **reimplementation of that app onto OCBM firmware**, and it lives beside sibling host apps in the
>    same `ccpa_custom/` project, so a name that only states brand + target platform distinguishes it
>    from neither. `MacHost/` states platform + role, in a tree where OCBM is already the given.
>
> **Nothing about the build changed** — the Xcode project, target and source dir remain `carlink_macOS`
> (that inner legacy name was deliberately left alone) and the shipped display name remains `Carlink`.
> Only the wrapper folder and the references to it moved. Verified after the rename: Swift harness
> `tests/run_tests.sh` 1413 passed / 0 failed; `carplay-iap2-core`'s cross-language contract test
> `metadata::tests::host_app_reads_every_emitted_key` (which reads `App/MetadataWindow.swift` by path)
> passes; `tools/proto_check.py` resolves `OCBM/OCBMFraming.swift` and reports OK.

Refactored from `carlink_macOS` (the original riddlebox-firmware CCPA host app) into
`ccpa_custom/host/MacHost/`. Goal
