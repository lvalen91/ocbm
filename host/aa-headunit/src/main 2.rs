//! aa-headunit — minimal Android Auto head-unit client (docs/androidauto/00_ARCHITECTURE.md).
//!
...
impl Link {
    ...
    /// message. A frame whose decrypt yields no plaintext (an empty TLS record —
    /// e.g. a lone alert/close_notify — is a protocol anomaly here, not a normal
    /// split) is treated as an error by `tls::decrypt` and ends the session via
    /// the `?` below, rather than being buffered for the next frame.
    ///
    ...
    fn recv(&mut self, mut tls: Option<&mut tls::HeadUnitTls>) -> std::io::Result<Msg> {
        loop {
            ...
            match frame_type {
                ...
                FT_FIRST => {
                    if let Some((_, old)) = self.partial.insert(channel, (encrypted, payload)) {
                        eprintln!("  .. FIRST on ch={channel} discarded {} in-flight bytes", old.len());
                    }
                }
                _ => {
    ...
    loop {
        ...
        match status {
            tls::HsStatus::Done => {
                ...
                tls.log_peer_certificate_issuer();
                break;
            }
