...
object Ocbm {
    ...
    const val PM_WIRED_CP: Byte = 0x01 // wired CarPlay (projection_up -> iap2d + carplayd)
    ...
    // RETIRED 2026-09-08 — rx-connect was merged into carplayd, so nothing emits this source any
    // more; its output is now /tmp/carplayd.log, i.e. LOG_SRC_AIRPLAYD. Kept so the numbering and
    // the name match ocbm-proto; do not wire a reader to it.
    ...
    /** RETIRED 2026-09-08 — the wireless half of [LOG_SRC_RX_CONNECT]; see the note there. */
    ...
    fun bhString(f: Int): String {
        val on = buildList {
            ...
            // The CONSTANT keeps its pre-rename name to match ocbm-proto (proto_check.py compares by
            // name); the LABEL is free to be current, and must be — it is what lands in the log line.
            if (f and BH_AIRPLAYD != 0) add("carplayd")
     ...
     *
     * Coordinates are NORMALIZED 0..=65535 across the display. carplayd scales them to absolute HID
    ...
    /**
     * "META". A fresh array per read, deliberately: `val` freezes the reference, not the contents,
     * so a shared instance is four bytes any caller can scribble over — and a corrupted magic does
     * not fail loudly, it silently stops every metadata frame from ever matching. Hoist it into a
     * local if you are matching in a loop.
     */
    val META_SEAM_MAGIC: ByteArray get() = byteArrayOf(0x4D, 0x45, 0x54, 0x41)

    /** Raw binary plist of an inbound iPhone `POST /command` ({type, params}) — from carplayd. */
...
object Mfi {
    ...
    fun parse(
        ...
        // [len] is a fill length, so it may be shorter than [payload] — but never longer. A caller
        // that gets this backwards would otherwise reach the copyOfRange below with rlen past the
        // end of the array and take an IndexOutOfBoundsException out of a protocol parser.
        if (len > payload.size) return null
