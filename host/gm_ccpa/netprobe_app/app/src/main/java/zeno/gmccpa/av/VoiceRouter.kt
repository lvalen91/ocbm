...
class VoiceRouter(
    ...
    /** AUs a sink silently discarded — oversized for the codec input buffer, or PCM left over when the
     *  AudioTrack buffer filled. Both used to be counted as routed audio. */
    ...
    enum class Purpose(
        ...
        ASSISTANT(AudioAttributes.USAGE_ASSISTANT, AudioAttributes.CONTENT_TYPE_SPEECH, "siri",
                  ASSISTANT_HOLD_MS + 1_000L, keepAlive = true),
    ...
    private companion object {
        ...
        /** Stop a sink whose stream has gone quiet this long, so its volume-group claim is released. */
        // Superseded by Purpose.idleMs — a single window across all purposes let CALL/MEDIA/ASSISTANT
        // contexts stay active simultaneously and confused the head unit's volume knob.
         ...
         * rate. Two keep-alive periods: enough that a silence write can never outrun the buffer.
         * Load-bearing since the voice entries began advertising 48 kHz (bit 32) alongside 16 kHz.
         */
    ...
    fun stop() {
        running.set(false)
        onDuck(false)
        log.i("stopping — ${framesDecoded.get()} AUs routed, ${ausDropped.get()} dropped, ${bytesIn.get()} bytes")
    }
    ...
    fun consume(ins: InputStream) {
        ...
        try {
            ...
            synchronized(sinks) { sinks.values.forEach { it.release() }; sinks.clear() }
            onDuck(false)
        }
    }
    ...
    private fun sweepIdle() {
        ...
        // Underruns are latched by release(), because the track is gone by the time we log outside the lock.
        val released = ArrayList<Pair<Purpose, Int>>()
        synchronized(sinks) {
            for ((p, s) in sinks) {
            ...
            dead.forEach { p -> sinks.remove(p)?.let { s -> s.release(); released.add(p to s.underruns) } }
        }
        released.forEach { (p, u) ->
            log.i("${p.label}: idle ${p.idleMs / 1000}s — released ($u track underruns; focus + volume group freed)")
        }
    ...
    private val ATYPE_COMPATIBILITY = 5
    ...
    private fun route(atype: Int, rate: Int, ch: Int, au: ByteArray) {
        ...
        if (sink.isConfigured && (sink.rate != rate || sink.channels != ch)) {
            ...
            sink.release(); sink.configure(rate, ch)
        }
        sink.feed(au)
    ...
    private inner class Sink(val p: Purpose) {
        ...
        fun keepAlive(now: Long) {
            ...
            // Upper bound is ASSISTANT_HOLD_MS, not idleMs: past that edge [assistantTick] has told
            // AacPlayer that Siri is done and media is coming back, and MUSIC outranks VOICE_COMMAND
            // in CarVolume's priority list — so silence-filling beyond it cannot win the knob and only
            // keeps a would-be-idle player active while the sink waits to be swept.
            if (quiet < KEEPALIVE_AFTER_MS || quiet > ASSISTANT_HOLD_MS) return
            ...
            try {
                ...
                tk = AudioTrack.Builder()
                    ...
                    .setBufferSizeInBytes(
                        maxOf(minBuf * 4, (r * TRACK_BUFFER_MIN_MS / 1000).toInt() * 2 * (if (c >= 2) 2 else 1)))
                    ...
                    if (ob != null && info.size > 0) {
                        ...
                        while (off < pcm.size) {
                            ...
                            if (w < 0) {
                                ...
                                release(); configureFailedAt = 0L
                                return
                            }
                            // Buffer full: drop the remainder, do not spin. Counted, because a sink that
                            // drops half of every AU is otherwise indistinguishable from a healthy one.
                            if (w == 0) { ausDropped.incrementAndGet(); break }
                            off += w
                        }
                    }
