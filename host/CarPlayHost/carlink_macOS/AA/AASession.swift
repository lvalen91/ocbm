...
final class AASession: @unchecked Sendable {
    ...
    // interval is 60 s at the protocol version we negotiate (2 s only at >= 6.0). Latency from a deep
    ...
    /// rejected outright (`CAR.SERVICE Critical error 2/24 "No audio/mic"`, docs/host/02_ANDROID_AUTO.md) — but nothing
    ...
    // Serializes all SSLContext ops (encrypt/decrypt) — the recv loop and a clean-quit
    // shutdown() from another thread must not touch the context concurrently.
    ...
    init?(transport: AAByteTransport, decoder: VideoDecoder, p12Path: String, p12Password: String,
          capability: AACapability, audio: AudioPlayer? = nil,
    ...
    private func sendMsg(_ channel: UInt8, _ msgid: UInt16, encrypted: Bool, control: Bool, _ body: Data) -> Bool {
        ...
        if encrypted {
            tlsLock.lock(); let ct = tls.encrypt(payload); tlsLock.unlock()
        ...
        while true {
            ...
            if encrypted {
                ...
                guard let d else {                // real TLS failure -> end the session
                    NSLog("[AA] decrypt FAILED on ch=\(channel) (\(payload.count) B frame). " +
    ...
    func run() {
        ...
        while true {
            guard let st = tls.handshakeStep() else { log("TLS handshake failed"); return }
            while let out = tls.takeOutbound() {
            ...
            tls.feedInbound(sslBody)
        }
        log("TLS handshake COMPLETE: \(tls.describe())")
        ...
        log("<- SERVICE_DISCOVERY_REQUEST; -> SERVICE_DISCOVERY_RESPONSE (7 services)")
        ...
        let sd = AAWire.serviceDiscoveryResponseFull(resolution: videoRes, fps: videoFps,
                                                     density: cap.density, tsW: videoW, tsH: videoH,
                                                     name: cap.name)
    ...
    private func eventLoop() {
        ...
        while true {
            ...
            if ch == AAWire.chMicrophone {
                ...
                case AAWire.micRequest:
                    ...
                    if open {
                        ...
                        micActive = false
                        log("<- MIC CLOSE (sent \(micFrames) frames)")
                        onMicStop?()
                    }
            ...
            if ch == AAWire.chMediaAudio || ch == AAWire.chGuidanceAudio || ch == AAWire.chSystemAudio {
                switch id {
            ...
            case (AAWire.chVideo, AAWire.mediaCodecConfig):
                // Annex-B SPS/PPS -> configure the decoder.
                if let (s, p) = AVCCFastPath.h264ParameterSetsFromAnnexB(body) {
                    ...
                    // CarPlay never touches maxDecodeDepth, so its default depth-1 lane is unchanged.
                ...
                _ = sendEnc(AAWire.chVideo, AAWire.mediaAck, control: false, AAWire.mediaAckBody(sessionId))
    ...
    private func containsIDR(_ raw: UnsafeRawBufferPointer) -> Bool {
        for r in AVCCFastPath.annexBNALRanges(raw) where r.lowerBound < raw.count {
            if (raw[r.lowerBound] & 0x1F) == 5 { return true }
        }
        return false
    }
    ...
    func sendMicPCM(_ pcm: Data) {
        guard micActive, !pcm.isEmpty else { return }
        ...
        micFrames += 1
        if micFrames == 1 || micFrames % 200 == 0 { log("mic frames=\(micFrames)") }
    }
        ...
        client.onIpData = { [weak self] id, bytes in
            ...
            if bytes.isEmpty {
                ...
                if self.rxLastLog.timeIntervalSinceNow <= -1 {
                    NSLog("[AA] CH_IP box->host total=\(self.rxTotal) (+\(bytes.count)B last, " +
                          "\(self.buffer.count)B unread)")
                    self.rxLastLog = Date()
                }
            }
            self.cond.signal()
    ...
    func readExact(_ n: Int) -> Data? {
        ...
        while buffer.count < n && !eof {
            if !cond.wait(until: deadline) {
                NSLog("[AA] read stall — no box data for \(Int(Self.readStallTimeout))s, giving up")
