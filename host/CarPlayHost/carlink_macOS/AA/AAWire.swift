...
// in main.rs), validated live against a Pixel (docs/host/02_ANDROID_AUTO.md). Foundation-only so the pure
...
enum AAWire {
    ...
    // AA protocol version: 1.7 = current DHU/gearhead ceiling (docs/host/02_ANDROID_AUTO.md).
    ...
    static func getFieldVarint(_ buf: Data, _ field: UInt32) -> UInt64? {
        ...
        while off < buf.count {
            ...
            case 0:
                ...
                if f == field { return v }
            case 2:
                guard let (len, o2) = getVarint(buf, off) else { return nil }
                off = o2 &+ Int(len)
            case 5: off += 4
            case 1: off += 8
            default: return nil
    ...
    /// Full ServiceDiscoveryResponse (the accepted 1.7 service set, docs/host/02_ANDROID_AUTO.md):
    ...
    static func serviceDiscoveryResponseFull(resolution: UInt32, fps: UInt32, density: UInt32,
                                             ...
                                             name: String = "Carlink") -> Data {
        // video sink id=3
        var vc = Data()
        putVarintField(&vc, 1, UInt64(resolution)); putVarintField(&vc, 2, UInt64(fps))
        putVarintField(&vc, 3, 0); putVarintField(&vc, 4, 0); putVarintField(&vc, 5, UInt64(density))
        var mssV = Data(); putVarintField(&mssV, 1, 3); putLenField(&mssV, 4, vc)
        ...
        let sinks = AACapability.audioSinks.map {
        ...
        putVarintField(&out, 6, 1) // driver_position = RIGHT
