...
final class AATLS {
    ...
    /// Our own GAL head-unit identity, kept only to read its certificate's issuer for the
    /// post-handshake peer-issuer log below. No verification decision reads this.
    private var ourIdentity: SecIdentity!
    ...
    init?(p12Path: String, password: String) {
        ...
        // MEMORY-ONLY import. Without this key, SecPKCS12Import writes the GAL head-unit identity —
        // including its PRIVATE KEY — into the user's login keychain, on every session start, where it
        // persists after the app quits and can raise keychain ACL prompts on a fresh machine. The
        // returned SecIdentity is still fully usable for this process's lifetime, which is all
        // SSLSetCertificate below needs. (macOS 15+; deployment target is well past that.)
        let opts = [kSecImportExportPassphrase as String: password,
                    kSecImportToMemoryOnly as String: kCFBooleanTrue as Any] as CFDictionary
        var itemsCF: CFArray?
    ...
    /// (peer verification stays disabled per SSLSetSessionOption(.breakOnServerAuth) above —
    /// this never fails the handshake) and logs its subject + whether its issuer matches our
    /// own GAL head-unit certificate's issuer. Call under `tlsLock`, before `tlsReady = true`
    /// (see AASession.swift), same as `describe()`.
    func logPeerCertificateIssuer() {
        var trustRef: SecTrust?
        guard SSLCopyPeerTrust(ctx, &trustRef) == noErr, let trust = trustRef else {
            NSLog("[AATLS] peer cert: SSLCopyPeerTrust failed"); return
        }
        guard let chain = SecTrustCopyCertificateChain(trust) as? [SecCertificate],
              let leaf = chain.first else {
            NSLog("[AATLS] peer cert: no certificate in peer trust"); return
        }
        let subject = (SecCertificateCopySubjectSummary(leaf) as String?) ?? "<unknown subject>"
        let peerIssuerSeq = SecCertificateCopyNormalizedIssuerSequence(leaf) as Data?

        var ourIssuerSeq: Data?
        if let ourIdentity, let ourCert = ourCertificate(ourIdentity) {
            ourIssuerSeq = SecCertificateCopyNormalizedIssuerSequence(ourCert) as Data?
        }

        if let peerIssuerSeq, let ourIssuerSeq, peerIssuerSeq == ourIssuerSeq {
            NSLog("[AATLS] peer cert subject=\"\(subject)\" issuer=GAL-issued (issuer matches head-unit cert)")
        } else {
            let issuerDesc = issuerName(of: leaf) ?? peerIssuerSeq.map { "0x" + $0.prefix(16).map { String(format: "%02x", $0) }.joined() } ?? "<unavailable>"
            NSLog("[AATLS] WARNING peer cert subject=\"\(subject)\" issuer=\"\(issuerDesc)\" does NOT match our head-unit cert's issuer (expected Google Automotive Link)")
        }
    }
}

/// Extracts the SecCertificate from a SecIdentity. Used only for the log-only issuer
/// comparison above.
private func ourCertificate(_ identity: SecIdentity) -> SecCertificate? {
    var certRef: SecCertificate?
    guard SecIdentityCopyCertificate(identity, &certRef) == errSecSuccess else { return nil }
    return certRef
}

/// Best-effort human-readable issuer name for a certificate, for the log-only peer-issuer
/// warning above. Never renders certificate bytes — only the parsed X.509 issuer RDNs.
private func issuerName(of cert: SecCertificate) -> String? {
    guard let values = SecCertificateCopyValues(cert, [kSecOIDX509V1IssuerName] as CFArray, nil) as? [String: Any],
          ...
          let rdns = issuerDict[kSecPropertyKeyValue as String] as? [[String: Any]] else {
        return nil
    }
    let parts = rdns.compactMap { rdn -> String? in
