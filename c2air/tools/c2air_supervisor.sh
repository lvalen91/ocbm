...
#
#   1. `RS_OPEN` is emitted when carplayd ACCEPTS a control connection, not at pair-verify
#      (crates/vendor/receiver/src/relay.rs:674-697, ccpa/carplayd/src/main.rs:1389).
#   2. The relay-vs-local DELEGATE IS CHOSEN ONCE, AT ACCEPT, from `appsetup() && seam_up()`
#      (ccpa/carplayd/src/main.rs:1386-1401). A control connection accepted while the RTSP seam is
...
# host-present 0->1 edge (tools/session_supervisor.sh:788-791), so btd — and therefore
# carplayd, and therefore any phone control connection — cannot exist before a subscribed host.
#
# The first C2Air bring-up inverted that: the stack started AP + BT + btd AT BOOT,
...
# bare `pkill` at :185, :193, :334 and :427 to reap carplayd. Every one of those spawns
...
#   * `pkill -x btd` can never match — Linux truncates comm to 15 chars
...
wireless_up() {
  ...
  # session so the new stack can never latch onto or collide with a stale carplayd.
  kill_path /tmp/carplayd
  sleep 1   # let the stragglers actually die before btd spawns its own

  echo "[sup] host present -> bringing wireless up at uptime=$(cut -d' ' -f1 /proc/uptime)"
  # BOX_HOSTAPD_CONF must name the SAME file c2air_ap_up.sh generated, or the 0x5703 reply
  # describes an AP that does not exist. State + peers go to UDISK because / is a read-only squashfs.
  BT_HCI_BACKEND=native \
  BOX_HOSTAPD_CONF=/tmp/hostapd.conf \
  BOX_STATE_DIR=/mnt/UDISK/cpstate \
  PEERSTORE_PATH=/mnt/UDISK/cpstate/carplay_peers.bin \
  AIRPLAYD_BIN=/tmp/carplayd \
    setsid "$CW" </dev/null >/tmp/cw.log 2>&1 &
  sleep 2
  wireless_running && echo "[sup] btd up" || echo "[sup] ERROR: btd failed to start"
}
