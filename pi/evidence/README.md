...
| `carplayd_key_lines.log` | The load-bearing lines from the box log | Contains `ASC f8f03000` (the fixed ELD config) and the SETUP dicts iOS sent. |
...
**The ELD fix, visible on the wire.** `carplayd_key_lines.log` shows `ASC f8f03000` — the 4-byte
